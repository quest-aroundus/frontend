  ...
  return (
    ...
    >
      <div className='mr-2.5 w-[calc(22.25rem-6.25rem-2rem)]'>
        <time className='text-main_b text-xs'>
          {useFormatDate(item.start_dt)}~{useFormatDate(item.end_dt)}
        </time>
        <h1 className='text-text_b font-bold text-xl leading-6'>
          {item.title}
        </h1>
        <p className='max-h-[2.375rem] text-wrap line-clamp-2 text-text_b !leading-[1.187rem]'>
          {item.description}
        </p>
        ...
        <div className='inline-flex items-center gap-1 mt-1'>
          ...
          <address
            ...
            className='text-wrap line-clamp-1 text-text_b text-xs not-italic'
          >
